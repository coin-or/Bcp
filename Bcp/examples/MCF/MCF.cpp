                 ...
                 const int argnum, const char * const * arglist)
{
  ...
  ifstream ifs(tm->par.entry(MCF_par::InputFilename).c_str());
  tm->data.readDimacsFile(ifs);
  return tm;
}
						 ...
						 BCP_pricing_status& pricing_status)
{
	...
	if (par.entry(MCF_par::AddDummySourceSinkArcs)) {
		...
		for (int i = 0; i < data.numcommodities; ++i) {
			...
			double w = data.commodities[i].demand * data.arcs[arcpos].weight;
...
{
	const BCP_generic_solution* gsol =
		dynamic_cast<const BCP_generic_solution*>(sol);
											 ...
											 BCP_vec<double>& cut_new_bd)
{
	...
	*/
	for (i = vars.size()-1; i >= 0; --i) {
		...
		if (!v) continue;
		const int vsize = v.getNumElements();
		const int* vind = v.getIndices();
		const int* vval = v.getElements();
		...
		for (j = bvars.size()-1; !violated && j >= 0; --j) {
			...
			int* pos = lower_bound(vind, vind+vsize, bv->arc_index);
			const double f = pos == vind+size ? 0.0 : vval[pos-vind];
			if (bv->ub() == 0.0) {
									   ...
									   const BCP_vec<BCP_cut*>& cuts)
{
	...
	for (i = vars.size()-1; i >= 0; --i) {
		...
		const int* vval = v->flow.getElements();
		for (j = 0; j < vsize; ++j) {
			f[vind[j]] += vval[j];
		}
	}
							...
							BCP_vec<BCP_col*>& new_cols)
{
	...
	int i;
	for (i = data.numarcs-1; i >= 0; --i) {
		cost[i] = dara.arcs[i].weight - pi[i];
	}
									...
									BCP_vec<BCP_lp_branching_object*>& cands)
{
	...
	for (int i = data.numcommodities-1; i >= 0; --i) {
		...
		if (most_frac_ind >= 0) {
			double lb = data.arcs[most_frac_ind].lb;
			double ub = data.arcs[most_frac_ind].ub;
			...
			new_vars.push_back(new MCF_branching_var(i, most_frac_ind,
													 lb, floor(frac_val),
													 floor(frac_val)+1, ub));
			cands.append(new BCP_lp_branching_object(2, // num of children
													 &new_vars,
													 NULL, // no new cuts
													 fvp,
													 NULL,
...
{
	MCF_var* v = dynamic_cast<const MCF_var*>(var);
	if (v) {
		v->pack(buf);
		return;
	}
	MCF_branching_var* bv = dynamic_cast<const MCF_var*>(var);
	if (bv) {
		bv->pack(buf);
		return;
	}
...
void MCF_data::pack(BCP_buffer& buf)
{
	...
	for (i = 0; i < numcommodities; ++i) {
		buf.pack(commodity[i].source);
		buf.pack(commodity[i].sink);
		buf.pack(commodity[i].demand);
	}
}

/*----------------------------------------------------------------------------*/

void MCF_data(BCP_buffer& buf)
{
	int namelen;
	...
	for (i = 0; i < numcommodities; ++i) {
		buf.unpack(commodity[i].source);
		buf.unpack(commodity[i].sink);
		buf.unpack(commodity[i].demand);
	}
}

//#############################################################################

void MCF_var::pack(BCP_buffer& buf)
{
	int type t = 0;
	buf.pack(t);
...
void MCF_branching_var::pack(BCP_buffer& buf)
{
	int type t = 1;
	buf.pack(t);
	buf.pack(commodity);
...
void MCF_branching_var(BCP_buffer& buf) :
...
BCP_var_algo* MCF_unpack_var(BCP_buffer& buf)
{
	...
	case 1: return new MCF__branching_var(buf);
...
int MCF_data::readDimacsFile(istream& s)
{
	const bool dummies = par.entry(MCF_par::AddDummySourceSinkArcs);
	double maxweight = 0;
	...
	while (s.good()) {
		...
		case 'p':
			...
			arcs = new arc[numarcs + (dummies ? numcommodities : 0)];
	...
	if (!size_read || arcs_read!=numarcs || commodities_read!=numcommodities) {
		printf("Input file is incorrect. size_read=%i arcs_read=%i commodities_read=%i\n",
			   size_read, network_read, commodities_read);
		return 1;
	}
