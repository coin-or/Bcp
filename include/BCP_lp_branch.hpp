...
public:
   ...
   /**@name Data members referring to forced bound changes. "Forced" means that
      the branching rule specifies this change. */
   /*@{*/
...
// NOTE: when the desctructor `delete's vars_to_add and cuts_to_add, it will
// just delete the pointers in the BCP_lp_..._sets (see the destructors of
// those classes). But this is intentional, because the vars/cuts will be
// actually deleted only later, when the unnecessery ones are deleted from the
// formulation.
...
public:
    ...
    */
   inline BCP_lp_branching_object* candidate() {return _candidate;}
   /** Return a const pointer to the candidate. */
   inline const BCP_lp_branching_object* candidate() const {return _candidate;}
   ...
   /**@name Modifying methods */
   /*@{*/
   /** Initialize the vector of actions to <code>BCP_ReturnChild</code> for
       every children. */
