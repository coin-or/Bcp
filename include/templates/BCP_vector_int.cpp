...
template<> void
BCP_vec<int>::insert_aux(iterator position, const_reference x){
   ...
   if (finish != end_of_storage) {
      ...
      *position = x;
      ...
      const size_t len = (2*size() + 0x1000) * sizeof(int);
...
template <>
BCP_vec<int>::BCP_vec(const size_t n, const_reference value) :
...
template <>
BCP_vec<int>::BCP_vec(const_iterator first, const_iterator last) :
   start(0), finish(0), end_of_storage(0)
{
...
template <>
BCP_vec<int>::BCP_vec(const int* x, const size_t num) :
   start(0), finish(0), end_of_storage(0)
{
   if (num > 0) {
      start = allocate(num);
...
template <> inline void
BCP_vec<int>::push_back(const_reference x) {
   if (finish != end_of_storage)
      *finish++ = x;
   else
      insert_aux(finish, x);
...
template <> inline void
BCP_vec<int>::unchecked_push_back(const_reference x) {
   *finish++ = x;
}

template <> inline void
BCP_vec<int>::pop_back() {
   --finish;
}
...
template <> void
BCP_vec<int>::keep(iterator pos) {
   *start = *pos;
   finish = start + 1;
}

template <> void
BCP_vec<int>::keep(iterator first, iterator last) {
   const size_t len = last - first;
...
template <> void
BCP_vec<int>::erase(iterator position) {
...
template <> void
BCP_vec<int>::erase(iterator first, iterator last) {
...
template<> void
BCP_vec<int>::reserve(const size_t n){
   if (capacity() < n) {
...
template<> void
BCP_vec<int>::swap(BCP_vec<int>& x) {
...
template<> BCP_vec<int>&
BCP_vec<int>::operator=(const BCP_vec<int>& x){
   if (&x != this) {
...
template<> void
BCP_vec<int>::assign(const void* x, const size_t num){
   if (num > capacity()){
      deallocate();
      start = allocate(num);
...
template<> void
BCP_vec<int>::insert(int* position, const void* first, const size_t n)
{
   if (n == 0) return;
   if ((size_t) (end_of_storage - finish) >= n) {
      ...
      const size_t new_size = (2*size() + n) * sizeof(int);
...
template<> void
BCP_vec<int>::insert(iterator position,
...
template<> void
BCP_vec<int>::insert(iterator position, const size_t n, const_reference x) {
   if (n == 0) return;
   if ((size_t) (end_of_storage - finish) >= n) {
      memmove(position + n, position, (finish - position) * sizeof(int));
      finish += n;
...
template<> void
BCP_vec<int>::unchecked_erase_by_index(BCP_vec<int>::const_iterator firstpos,
				       BCP_vec<int>::const_iterator lastpos) {
   if (firstpos == lastpos)
      return;
   --lastpos;
   ...
   while (firstpos != lastpos){
      ...
      ++firstpos;
      if (*firstpos > source) {
	 memmove(target, start + source, (*firstpos - source) * sizeof(int));
	 target += (*firstpos - source);
      }
   }
   source = *firstpos + 1;
   int oldsize = size();
   memmove(target, start + source, (oldsize - source) * sizeof(int));
...
template<> void
BCP_vec<int>::unchecked_erase_by_index(const BCP_vec<int>& positions){
...
template<> void
BCP_vec<int>::erase_by_index(const BCP_vec<int>& positions){
   BCP_vec_sanity_check(positions.begin(), positions.end(), size());
   unchecked_erase_by_index(positions.begin(), positions.end());
}
...
template<> void
BCP_vec<int>::erase_by_index(BCP_vec<int>::const_iterator firstpos,
			     BCP_vec<int>::const_iterator lastpos) {
   BCP_vec_sanity_check(firstpos, lastpos, size());
...
template<> void
BCP_vec<int>::update(const BCP_vec<int>& positions,
			const BCP_vec<int>& values){
